<template>
    <li class="item">
       <div class="user-box">
        ...
        <button v-if="currentUser && currentUser.uid == user.id" v-on:click="showBtns = !showBtns">
             <fa icon="ellipsis-v" />
        </button>
        ...
        <div v-if="showBtns" class="controls">
            ...
            <li v-on:click="deleteWhisper" style="color:red">
                delete
            </li>
        </div>
    </li>
...
export default {
    ...
    firestore (){
        return{
            ...
            //ここで使われてるuidはwhisperのuid、このuidを使って、whisperを投稿したuserを判定してる。
    ...
    methods:{
        ...
        updateWhisper(){
            const date = new Date()
            db.collection('whispers').doc(this.$props.id).set({
